...
#             if i % 50 == 0:  # 每50次迴圈打印一次進度
#                     print(f"------------Processing index {i} / {len(signals)}------------")
#             #cointegration test
#             coint_status,model=method(signals['asset1'].iloc[i-bandwidth:i],
#                                       signals['asset2'].iloc[i-bandwidth:i])
#             print(f"------------Index {i}: Cointegration status = {coint_status}------------")                
...
#                 if signals.at[signals.index[i-1], 'signals1'] != 0:
#                     signals.loc[signals.index[i], 'signals1'] = 0
#                     signals.loc[signals.index[i], 'signals2'] = 0
#                     signals.loc[i:, 'z'] = np.nan
#                     signals.loc[i:, 'z upper limit'] = np.nan
#                     signals.loc[i:, 'z lower limit'] = np.nan
#                     signals.loc[i:, 'fitted'] = np.nan    
#                     signals.loc[i:, 'residual'] = np.nan
...
#                     signals.loc[i:, 'fitted'] = model.predict(sm.add_constant(signals['asset1'].iloc[i:]))
#                     signals.loc[i:, 'residual'] = signals['asset2'].iloc[i:] - signals.loc[i:, 'fitted']
...
#                     signals.loc[i:, 'z'] = (signals.loc[i:, 'residual'] - np.mean(model.resid)) / np.std(model.resid)
...
#                     signals.loc[i:, 'z upper limit'] = signals.loc[i:, 'z'] + np.std(model.resid)
#                     signals.loc[i:, 'z lower limit'] = signals.loc[i:, 'z'] - np.std(model.resid)
...
#         return signals
#     except Exception as e:
#         print(f"------------Error in signal_generation: {e}------------")
#         return None
...
#     #create signals
#     signals=signal_generation(asset1,asset2,EG_method,250)
#     if signals['z'].dropna().empty:
...
def main():
    ...
    days=int(calculate_days(stdate,eddate)/3)
    print('-----------------------'+str(days)+'--------------------------')
    #create signals
    signals=signal_generation(asset1,asset2,EG_method,days)
