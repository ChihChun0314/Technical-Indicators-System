...
def main():
    ...
    try:
        # 讀取台灣50指數的股票資料
        stocks = pd.read_csv('PythonScripts\quantitative_momentum\Y_taiwan_50_index.csv')
        ...
        final_dataframe = final_dataframe[:Remove_Low_Momentum_Stocks_Number+1]
        ...
        for symbol_string in symbol_strings:
            print(f"Downloading data for: {symbol_string}")
            data = yf.download(symbol_string, period="1y", interval="1d", group_by='ticker', auto_adjust=True)
            # time.sleep(2)  # 等待2秒
            print(f"Downloaded data for: {symbol_string}")
            for symbol in symbol_string.split():
